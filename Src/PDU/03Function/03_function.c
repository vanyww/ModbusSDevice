                                                   ...
                                                   size_t *replySize)
{
   ...
   ModbusFunction03RequestData *request = handle->Dynamic.ReceiveBufferFunctionSpecificData;

   if(request->RegistersToReadCount > __MODBUS_FUNCTION_03_MAX_REGISTERS_COUNT)
      return MODBUS_SDEVICE_STATUS_ILLEGAL_DATA_ERROR;

   ModbusFunction03ReplyData *reply = handle->Dynamic.TransmitBufferFunctionSpecificData;
